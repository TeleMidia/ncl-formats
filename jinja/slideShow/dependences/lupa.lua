...
--Replace original lpeg support for LuLPeg library.
--local lpeg = require('lpeg')

local lulpeg = require"lulpeg"
local re = lulpeg.re

lulpeg.locale(lulpeg)
local space, newline = lulpeg.space, lulpeg.P('\r')^-1 * '\n'
local P, S, V = lulpeg.P, lulpeg.S, lulpeg.V
local C, Cc, Cg, Cp, Ct = lulpeg.C, lulpeg.Cc, lulpeg.Cg, lulpeg.Cp, lulpeg.Ct
...
-- Returns an lulpeg pattern that immediately raises an error with message
-- *errmsg* for invalid syntax when parsing a template.
-- @param errmsg The error message to raise an error with.
local function lulpeg_error(errmsg)
...
function M.configure(ts, te, vs, ve, cs, ce, options)
  ...
  -- Error messages.
  local variable_end_error = lulpeg_error('"'..ve..'" expected')
  local comment_end_error = lulpeg_error('"'..ce..'" expected')
  local tag_end_error = lulpeg_error('"'..te..'" expected')
  local endraw_error = lulpeg_error('additional tag or "'..ts..' endraw '..te..
                                  '" expected')
  local expr_error = lulpeg_error('expression expected')
  local endblock_error = lulpeg_error('additional tag or "'..ts..' endblock '..
                                    te..'" expected')
  local endfor_error = lulpeg_error('additional tag or "'..ts..' endfor '..te..
                                  '" expected')
  local endif_error = lulpeg_error('additional tag or "'..ts..' endif '..te..
                                 '" expected')
  local endmacro_error = lulpeg_error('additional tag or "'..ts..' endmacro '..
                                    te..'" expected')
  local endcall_error = lulpeg_error('additional tag or "'..ts..' endcall '..te..
                                   '" expected')
  local endfilter_error = lulpeg_error('additional tag or "'..ts..' endfilter '..
                                     te..'" expected')
  local tag_error = lulpeg_error('unknown or unexpected tag')
  local main_error = lulpeg_error('unexpected character; text or tag expected')
...
local function expand(template, env, source)
  ...
  M._FILENAME = source -- for lulpeg errors only
  local ast = assert(lulpeg.match(M.grammar, template), "internal error")
...
function M.filters.urlize(s, length, nofollow)
  ...
  local patt = lulpeg.Cs(lead * middle * trail / function(lead, middle, trail)
    local linked
